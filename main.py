...
def main():
    ...
    landmarks = read_world("./world.dat") #read off landmarks

    #initialize the particles
    map_limits = [0, 10, 0, 10]
    particles = initialize_particles(1000, map_limits)

    actual_robot_pos = read_pos("./pos.dat") #you can change this and the world if you want

    #run particle filter
    for iteration in range(100):      #this part is for plotting, not very important.   
        sensor_readings = 1           #there is a little bit of odometry here too
        ...
        actual_robot_pos = update_robot_pos(actual_robot_pos, odometry, map_limits) #where the robot actually is
        sensor_readings = update_sensor_readings(actual_robot_pos, landmarks) #where the odometry says the robot is

        #predict particles by sampling from motion model with odometry info
        new_particles = sample_motion_model(odometry, particles) #completed for us

        #calculate importance weights according to sensor model
        weights = eval_sensor_model(sensor_readings, new_particles, landmarks) #to be completed
